 ...
 *   interactiveX V2
 * Mofified for Moto**** by whirleyes
 */
...
SYMSEARCH_DECLARE_FUNCTION_STATIC(int, sched_setscheduler_nocheck_s, struct task_struct *, int, const struct sched_param *);
...
{
	SYMSEARCH_BIND_FUNCTION_TO(interactivex, cpu_up, cpu_up_s);
	SYMSEARCH_BIND_FUNCTION_TO(interactivex, sched_setscheduler_nocheck, sched_setscheduler_nocheck_s);
	unsigned int i;
	struct cpufreq_interactivex_cpuinfo *pcpu;
	...
	sched_setscheduler_nocheck_s(up_task, SCHED_FIFO, &param);
...
MODULE_AUTHOR("Mike Chan <mike@android.com>, imoseyon");
MODULE_DESCRIPTION("'cpufreq_interactiveX V2' - Interactive governor + early suspend & hotplugging when the screen is off.\nextra : Moto**** ported version.");
MODULE_LICENSE("GPL");
